...
plan <- drake_plan(
  fm=target(prepare_feature_matrix(file_in(!!peak),norm_shift=10),transform = map(peak=!!peaks)),# convert peaks to feature matrix
  diag_stat=target(fm %>% select(diagnosis) %>% unique %>% length,transform = map(fm)),
  results = target(

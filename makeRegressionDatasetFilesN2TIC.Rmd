---
title: "Prepare dataset for regressionN2TIC"
...
#Log file 
logname<-'RegressionMetaDataN2TIC.spec.log'
#Database export folder to get data from
sdir<-'/var/workspaceR/scalpelData/peaksPat_isID5/' #'peakPat'
#Target folder for regression data 
tdir<-'/var/workspaceR/scalpelData/regression202104/data'
#Signal-to-noise threshold. Database have all peaks with SNR>1
SNR<-2
if(!file.exists(paste0(tdir,'/regressionN2TIC'))){
  dir.create(paste0(tdir,'/regressionN2TIC'))
}
```
...
for(i in 1:mtN){
  ...
  cat(format(Sys.time(), "%b %d %X"),i,mtN,'file read\n',file = logname,append = TRUE)
    ...
    dtm<-dt[pidx]
    fwrite(dtm,file = paste0(tdir,'/regressionN2TIC/',fnm),sep = '|',col.names = FALSE)
  cat(format(Sys.time(), "%b %d %X"),i,mtN,'TSV written\n',file = logname,append = TRUE)
    dtSNR<-dtm[snr>SNR]
    dtSNR$orig.intensity<-dtSNR$intensity
    dtSNR$intensity<-dtSNR$norm2tic*1e9
  ...
  cat(format(Sys.time(), "%b %d %X"),i,mtN,'peak list ready\n',file = logname,append = TRUE)
    ...
    saveRDS(p,file = paste0(tdir,'/regressionN2TIC/',fprds))
  ...
  cat(format(Sys.time(), "%b %d %X"),i,mtN,'MZ0 ready\n',file = logname,append = TRUE)
    ...
    saveRDS(dtm0,file = paste0(tdir,'/regressionN2TIC/',fprds))
  ...
  cat(format(Sys.time(), "%b %d %X"),i,mtN,'MZ2 ready\n',file = logname,append = TRUE)
      ...
      saveRDS(dtm2,file = paste0(tdir,'/regressionN2TIC/',fprds))
